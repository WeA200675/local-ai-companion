...
def build_system_prompt(persona: PersonaState, preference_tags: Iterable[str] = ()) -> str:
    """Compile editable persona state into a model-agnostic system prompt.

    Preference tags are deliberately treated as user configuration rather than
    hard-coded behavior. This keeps persona learning separate from the model and
    makes the prompt compiler replaceable later.
    """
